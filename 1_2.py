...
while(True):
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("facedata1/" + str(face_id) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w])
    ...
    if os.path.isfile("facedata1/" + str(face_id) + '.' + "1.jpg") == False:
        ...
        exit()
    for i in range(1,5):
        filePath = "facedata1/" + str(face_id) + '.' + str(count) + ".jpg"
...
if rooms is not None:
    ...
    itchat.send(" 人脸： " + str(face_id) + " 采集成功！",toUserName=username)
else:
    print('\n [提示]发送的群名有误！')

print(" [提示] 人脸：" + str(face_id) + " 采集成功！")
print("\n [提示] 正在退出程序...")
cam.release()
